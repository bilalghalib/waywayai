...
const getBase64Image = (url) => {
    ...
    img.onload = () => {
        ...
        const ctx = canvas.getContext("2d");c
...
function saveDynamicDataToFile() {
    ...
    userInput = compressToUTF16(userInput)
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (err) {
                alert("Sorry, upload failed - " + err.error + "- " + err.message);
                console.error(err);
                return;
            }
            records.forEach(function(record) {
...
// drawing play functions
function pprevDrawing() {
    alert("pp");
}
...
function runDrawing() {
    ...
    if(txtUrl == "") {
        document.getElementById("statusText").innerHTML = "Enter the url you want to play!";
        return;
    }

    runFlag = 1 - runFlag;
    if(runFlag) {
        document.getElementById("runDrawingBtn").innerHTML = "||";
        changeSpeed();
    } else {
        document.getElementById("runDrawingBtn").innerHTML = "&gt;";
    ...
    response.text().then(function(text) {
        drawingJsonStr = LZString.decompressFromUTF16(text);
        console.log(drawingJsonStr)
        done();
    });
});
